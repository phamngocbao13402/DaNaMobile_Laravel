                                    ...
                                    <tr class="inner-box">
                                        ...
                                        <td>
                                            <div class="event-wrap">
                                                <h3><a href="#">{{$voucher->code}}</a></h3>
                                                <div class="meta">
                                                    <div class="">
                                                        <a href="#">{{$voucher->type}}</a>
                                                    </div>
                                                    {{-- <div class="categories">
                                                        <a href="#">Inspire</a>
                                                    </div> --}}
                                                    <div class="time">
                                                        <span>Hạn sử dụng: {{$voucher->time}}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="primary-btn">
												@if (!Auth::check()) 
													<a type="submit" class="btn btn-primary" href="{{route('login')}}">Nhận ngay</a>
												@else
												
													@if($voucher->isGet)
														<a class="btn btn-primary" style="color: white">Bạn đã nhận</a>
													@else
														<form action="{{url('/voucher/addVoucher')}}" method="post">
															@csrf
															<input type="hidden" name="voucher_id" value="{{$voucher->id}}">
															<input type="hidden" name="numberof" value="1">
															<input type="hidden" name="user_id" value="{{Auth::user()->id}}">
															<button type="submit" class="btn btn-primary" href="{{url('voucher/addVoucher')}}">Nhận ngay</button>
														</form>
